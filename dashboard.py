...
(function(){{
  ...
  function initMap() {{
    if (map) return;
    map = L.map('map', {{ scrollWheelZoom: false }}).setView([51.5, 5.0], 7);
    L.tileLayer('https://{{s}}.tile.openstreetmap.org/{{z}}/{{x}}/{{y}}.png', {{
      attribution: '© OpenStreetMap', maxZoom: 18
    }}).addTo(map);

    // Twee vingers scrollen → pan; knijpen (ctrlKey) → zoom
    map.getContainer().addEventListener('wheel', e => {{
      ...
      if (e.ctrlKey) {{
        ...
        // Scrollen: verschuif de kaart
        map.panBy([e.deltaX * 1.5, e.deltaY * 1.5], {{animate: false}});
      }}
    }}, {{passive: false}});
  ...
  clearBtn.addEventListener('click', () => {{
    ...
    cards.forEach(c=>{{
      ...
      const d=c.querySelector('.card-distance'); if(d) d.textContent='';
    }});
